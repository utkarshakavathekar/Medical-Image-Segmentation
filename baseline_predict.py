...
if __name__ == '__main__':
    ...
    """ testing on original traning dataset"""
    ...
    """
    val_data = list(zip(sorted(glob('../input/DRIVE/STARE/2/test/images/*.tif')),
                          sorted(glob('../input/DRIVE/STARE/2/test/2nd_manual/*.gif')),
                        sorted(glob('../input/DRIVE/STARE/2/test/mask/*.gif'))))

    """
    """testing on fundus-images dataset """
    """
    val_data = list(zip(sorted(glob('../input/DRIVE/fundus-images/test/images/*.tif')),
                          sorted(glob('../input/DRIVE/fundus-images/test/2nd_manual/*.gif')),
                        sorted(glob('../input/DRIVE/fundus-images/test/mask/*.gif'))))
    """
    """testing on CHASE-Segmentation dataset """
    """
    val_data = list(zip(sorted(glob('../input/DRIVE/CHASE_Segmentation/2/test/images/*.tif')),
                          sorted(glob('../input/DRIVE/CHASE_Segmentation/2/test/2nd_manual/*.gif')),
                        sorted(glob('../input/DRIVE/CHASE_Segmentation/2/test/mask/*.gif'))))
    """
    try:
    ...
    for batch_files in tqdm(batch(val_data), total=len(val_data)//batchsize):
        ...
        for i, image_path in enumerate(batch_files):
            ...
            pred_ = 1.*(pred_ - np.min(pred_))/(np.max(pred_)-np.min(pred_))

            image_base = image_path[0].split("/")[-1]
            imb = image_base.replace("\\", "/")
            print(imb)
            cv2.imwrite("../output/"+model_name+"test/"+imb, pred_)
